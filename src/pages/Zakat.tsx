...
import ZakatCalculatorTab from "@/components/calculators/ZakatCalculatorTab";

export default function Zakat() {
  const [searchParams] = useSearchParams();
  const tab = searchParams.get("tab");

  // Backward-compat: redirect old tab URLs to new dedicated routes
  if (tab === "tatheer") return <Navigate to="/tatheer" replace />;
  if (tab === "khums") return <Navigate to="/khums" replace />;
  if (tab === "sadaqah") return <Navigate to="/giving" replace />;
      ...
      <div className="container relative z-10 mx-auto max-w-3xl">
        <div className="mb-6 text-center">
          ...
          <p className="mt-3 text-muted-foreground">
            Calculate your zakat obligation with madhab-specific guidance
          </p>
        </div>
        ...
        <ZakatCalculatorTab onAmountChange={handleAmountChange} />

        {/* Charity Section — shown when amount > 0 */}
        {activeAmount > 0 && (
          <CharitySection activeTab="zakat" amount={activeAmount} />
        )}
      </div>
    </main>
  );
